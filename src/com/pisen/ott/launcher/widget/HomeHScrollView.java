 ...
 * allowing it to be larger than the physical display. A BlockHScrollView is a
 ...
 * {@link LinearLayout} in a horizontal orientation, presenting a horizontal
 * array of top-level items that the user can scroll through.
 *
 * <p>
 * The {@link TextView} class also takes care of its own scrolling, so does not
 * require a BlockHScrollView, but using the two together is possible to achieve
 * the effect of a text view within a larger container.
 *
 * <p>
 * BlockHScrollView only supports horizontal scrolling. For vertical scrolling,
 * use either {@link ScrollView} or {@link ListView}.
 *
 * @attr ref android.R.styleable#BlockHScrollView_fillViewport
 */
public class HomeHScrollView extends FrameLayout {
	static final int ANIMATED_SCROLL_GAP = 300;
	...
	public HomeHScrollView(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		initScrollView();
	}

	public HomeHScrollView(Context context, AttributeSet attrs, int defStyle) {
	...
	private void initScrollView() {
		mScroller = new OverScroller(getContext(), new LinearInterpolator()/*, new ViscousFluidInterpolator()*/);
		setFocusable(false);
	...
	public void addView(View child) {
		if (getChildCount() > 0) {
			throw new IllegalStateException("BlockHScrollView can host only one direct child");
		}
	...
	/**
	 * @return Returns true this BlockHScrollView can be scrolled
	 */
	...
	/**
	 * Indicates whether this BlockHScrollView's content is stretched to fill
	 * the viewport.
	 *
	...
	/**
	 * Indicates this BlockHScrollView whether it should stretch its content
	 ...
	 * ((event.getSource() & InputDevice.SOURCE_CLASS_POINTER) != 0) { switch
	 * (event.getAction()) { case MotionEvent.ACTION_SCROLL: { if
	 * (!mIsBeingDragged) { final float hscroll; if ((event.getMetaState() &
	 * KeyEvent.META_SHIFT_ON) != 0) { hscroll =
	 * -event.getAxisValue(MotionEvent.AXIS_VSCROLL); } else { hscroll =
	 * event.getAxisValue(MotionEvent.AXIS_HSCROLL); } if (hscroll != 0) { final
	 * int delta = (int) (hscroll * getHorizontalScrollFactor()); final int
	 * range = getScrollRange(); int oldScrollX = getScrollX(); int newScrollX =
	 * oldScrollX + delta; if (newScrollX < 0) { newScrollX = 0; } else if
	 * (newScrollX > range) { newScrollX = range; } if (newScrollX !=
	 * oldScrollX) { super.scrollTo(newScrollX, getScrollY()); return true; } }
	...
	public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
		...
		info.setClassName(HomeHScrollView.class.getName());
	...
	public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
		...
		event.setClassName(HomeHScrollView.class.getName());
	 ...
	 *
	 * @param leftFocus
	 *            look for a candidate is the one at the left of the bounds if
	 *            leftFocus is true, or at the right of the bounds if leftFocus
	 *            is false
	 * @param left
	 ...
	 *            found
	 * @param right
	 *            the right offset of the bounds in which a focusable must be
	 *            found
	 * @return the next focusable component in the bounds or null if none can be
	 *         found
	 */
	 ...
	 *
	 * @param direction
	 *            the scroll direction: {@link android.view.View#FOCUS_LEFT} to
	 *            go one page left or {@link android.view.View#FOCUS_RIGHT} to
	 *            go one page right
	 ...
	 * leftmost/rightmost component in the new visible area. If no component is
	 * a good candidate for focus, this scrollview reclaims the focus.
	 * </p>
	 *
	 * @param direction
	 *            the scroll direction: {@link android.view.View#FOCUS_LEFT} to
	 *            go the left of the view or
	 *            {@link android.view.View#FOCUS_RIGHT} to go the right
	 * @return true if the key event is consumed by this method, false otherwise
	 */
	...
	/**
	 * <p>
	 * Scrolls the view to make the area defined by <code>left</code> and
	 * <code>right</code> visible. This method attempts to give the focus to a
	 ...
	 * @param direction
	 *            the scroll direction: {@link android.view.View#FOCUS_LEFT} to
	 *            go left {@link android.view.View#FOCUS_RIGHT} to right
	 * @param left
	 *            the left offset of the new area to be made visible
	 * @param right
	 *            the right offset of the new area to be made visible
	 * @return true if the key event is consumed by this method, false otherwise
	 */
