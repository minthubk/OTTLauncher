...
//  private int mVideoChroma = MediaPlayer.VIDEOCHROMA_RGBA;
  private int mVideoChroma = MediaPlayer.VIDEOCHROMA_RGB565;
  ...
  private String mVideoShortName;
  private Paint paint ;
  ...
  private OnErrorListener mErrorListener = new OnErrorListener() {
    public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
      ...
      if (getWindowToken() != null) {
        ...
        .getIdentifier("VideoView_error_title", "string", mContext.getPackageName()))
        .setMessage(message)
...
//            }
  ...
  public void setVideoLayout(int layout, float aspectRatio) {
    ...
    Log.e("VIDEO: %dx%dx%f, Surface: %dx%d, LP: %dx%d, Window: %dx%dx%f", mVideoWidth, mVideoHeight, mVideoAspectRatio, mSurfaceWidth, mSurfaceHeight, lp.width, lp.height, windowWidth, windowHeight, windowRatio);
    mVideoLayout = layout;
...
//      mMediaPlayer.setBufferSize(mBufSize);
      ...
      attachMediaController();  //绑定媒体控件播放器
  ...
  private void attachMediaController() {
    if (mMediaPlayer != null && mMediaController != null) {
      ...
      if (mUri != null) {
	        ...
	        mVideoShortName = name.substring(0, name.lastIndexOf('.'));
	        mMediaController.setFileName(mVideoShortName);
      }
    }
  }
  
  public String getVideoShortName(){
		 return mVideoShortName;
	 }
  ...
  public void toggleVolumControlsVisiblity(){
	  if(!mMediaController.isShowing()){
		   mMediaController.show();
	  }
  }
  ...
  /**
   * 视频播放暂停,要显示暂停大图标,显示控制进度条,更换播放图标
   */
  public void pause() {
    if (isInPlaybackState()) {
